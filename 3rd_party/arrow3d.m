function varargout=arrow3d(start,stop,ang,ltype,p,n, pgColor)
%   Plot 3d Arrows
% 
...
if strcmp(ltype,'line')
    ...
    for i=1:length(dis)
        ...
        hlines(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',pgColor);
        hlgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:), pgColor);
    end
end
...
for i=1:length(dis)
    ...
    [rx,ry,rz] = rotatedata(xi,yi,zi,nvec(i,:),cosrang(i),[0,0,0]);
    cx=start(i,1)+rx;cy=start(i,2)+ry;cz=start(i,3)+rz;
    hheads(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',pgColor);
    %Draw the underside of taper
    hhgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:), pgColor);
end
if SET_HOLD_OFF
    hold off;
end
